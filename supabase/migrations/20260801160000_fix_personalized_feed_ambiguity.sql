-- Fix 42702 "column reference id is ambiguous" in get_personalized_feed
-- The bare `id` in `where id = v_user_id` conflicts with RETURNS TABLE OUT param `id`
...
begin
  ...
  -- Get user's county - FIX: qualify id as profiles.id
