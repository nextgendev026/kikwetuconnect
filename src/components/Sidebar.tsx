...
export default function Sidebar({ initials, profile, following = [], onlineIds = new Set<string>(), collapsed = false }: {
  ...
  // Collapse wrap — shows brand, nav and mini only; nav becomes icon rail
  return (
    <aside className="sidebar animate-slide-in-left">
      ...
      {/* Your network — Following with internal scroll */}
      <div className="live-users" style={{ marginTop: 12, padding: '0 4px', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8, padding: '0 2px' }}>
          <span style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--green)', boxShadow: '0 0 5px var(--green)' }} />
          <small style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', color: 'var(--muted)' }}>
            Following · {following.filter(u => onlineIds.has(u.id)).length} online
          </small>
        </div>
      ...
      {/* User mini */}
      <div className="user-mini" style={{ marginTop: 'auto', flex: 'none' }}>
